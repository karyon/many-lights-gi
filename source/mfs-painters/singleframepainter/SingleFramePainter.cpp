...
SingleFramePainter::SingleFramePainter(ResourceManager & resourceManager, const cpplocate::ModuleInfo & moduleInfo)
: Painter("SingleFramePainter", resourceManager, moduleInfo)
, m_animation(true)
...
, m_angle(0.0f)
{
    ...
    // Register properties
    addProperty<bool>("Animation", this, &SingleFramePainter::animation, &SingleFramePainter::setAnimation);
    addProperty<Color>("BackgroundColor", this, &SingleFramePainter::background, &SingleFramePainter::setBackground);
    addProperty<FilePath>("Texture", this, &SingleFramePainter::texture, &SingleFramePainter::setTexture);
}

SingleFramePainter::~SingleFramePainter()
{
}

bool SingleFramePainter::animation() const
{
    return m_animation;
}

void SingleFramePainter::setAnimation(const bool & enabled)
{
    m_animation = enabled;
...
Color SingleFramePainter::background() const
{
    return m_background;
}

void SingleFramePainter::setBackground(const Color & color)
{
    m_background = color;
}

FilePath SingleFramePainter::texture() const
{
    return m_textureFilename;
}

void SingleFramePainter::setTexture(const FilePath & filename)
{
...
void SingleFramePainter::onInitialize()
{
#ifdef __APPLE__
...
void SingleFramePainter::onPaint()
{
...
void SingleFramePainter::createAndSetupCamera()
{
    m_camera = new Camera();
    m_camera->setEye(glm::vec3(0.0, 0.0, 12.0));
}

void SingleFramePainter::createAndSetupTexture()
{
    // Try to load texture
